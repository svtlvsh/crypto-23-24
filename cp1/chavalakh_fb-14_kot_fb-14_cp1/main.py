...
def calculate_entropy_bigr(freq, cross = 0):
    entropy = 0.0
    for probability in freq.values():
        if probability > 0:
            entropy -= probability * math.log2(probability)
    return entropy / (cross + 1)
...
def xlsx_save(filename, letters = 1, bigrams = 1):
    ...
    wb.save(f'C:/Users/Catto/Desktop/Крипта/{filename}.xlsx')

with open(f'C:/Users/Catto/Desktop/Крипта/text.txt', 'r', encoding='utf-8') as file:
    text = file.read()
...
print(f'Bigrams entropy: {calculate_entropy_bigr(bigram_freq, 0)}')
...
print(f'Bigrams entropy: {calculate_entropy_bigr(bigram_freq, 1)}')
print(f'Bigrams redundancy: {calculate_redundancy(calculate_entropy_bigr(bigram_freq, 1), 1)}\n')
